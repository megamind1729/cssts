...
import history.*;
import history.ParseHistoryError;
import util.Pair;
...
public class PartialOrderClosure {
    ...
    public static IncrementalCSSTs Closure(History history2) throws ParseHistoryError {
        
        long startTime = System.nanoTime(); System.err.println("[ Closure ] startTime: " + startTime / 1_000_000.0 + " ms");    
            
        // Initialization

        ReducedHistory history = history2.reduceHistory();
        // System.out.println("\nA." + history); // Debugging
        ...
        for (int sIdx = 0; sIdx < width; sIdx++) {
            for (int tIdx = 0; tIdx < sessionLengths[sIdx]; tIdx++) {
                ReducedTransaction t = history.sessions.get(sIdx).get(tIdx);
                for (Map.Entry<Key, Event.Read> entry : t.firstReads.entrySet()) {
                    ...
                    if (w != null) {
                        ...
                        if(Objects.equals(w.getSessTxnPair(), sIdx_tIdx)){
                            System.out.println("w and r are in same transaction. History not properly processed. w : " + w + ", r : " + e); // Debugging
                        }
    ...
    private static void ObsClosure(Pair<Integer, Integer> e1, Pair<Integer, Integer> e2, Queue<Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> Q, ReducedHistory history, EventMaps eventMaps) {
